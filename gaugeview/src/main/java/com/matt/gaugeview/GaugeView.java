...
public class GaugeView extends View {
    ...
    public static boolean SHOW_TEXT = false;
    ...
    public static final float SCALE_START_ANGLE = 50.0f;
    ...
    public static final float[] RANGE_VALUES = {250.0f, 500.0f, 750.0f, 1000.0f};
    ...
    private float mAccelerationCoefficient = 5.0f;
    ...
    private String mTextUnitBelow;
    protected String mTextUnitLabel;
    ...
    public boolean getShowOuterRim(TypedArray a) {
        return a.getBoolean(R.styleable.GaugeView_showOuterRim, SHOW_OUTER_RIM);
    }

    public boolean getShowInnerRim(TypedArray a) {
        return a.getBoolean(R.styleable.GaugeView_showInnerRim, SHOW_INNER_RIM);
    }

    public float getScaleStartAngle(TypedArray a) {
        return a.getFloat(R.styleable.GaugeView_scaleStartAngle, SCALE_START_ANGLE);
    }

    public int getScaleDivisions(TypedArray a) {
        return a.getInteger(R.styleable.GaugeView_divisions, SCALE_DIVISIONS);
    }

    public int getScaleSubdivisions(TypedArray a) {
        return a.getInteger(R.styleable.GaugeView_subdivisions, SCALE_SUBDIVISIONS);
    }

    public float getDefaultNeedleHeight(TypedArray a) {
        return a.getFloat(R.styleable.GaugeView_needleHeight, NEEDLE_HEIGHT);
    }
    ...
    private void readAttrs(final Context context, final AttributeSet attrs, final int defStyle) {
        ...
        mShowOuterRim = getShowOuterRim(a);
        mShowInnerRim = getShowInnerRim(a);
        ...
        mNeedleWidth = getDefaultNeedleWidth(a);
        mNeedleHeight = getDefaultNeedleHeight(a);
        ...
        mScaleStartAngle = getScaleStartAngle(a);
        mScaleEndAngle = a.getFloat(R.styleable.GaugeView_scaleEndAngle, 360.0f - mScaleStartAngle);

        mDivisions = getScaleDivisions(a);
        mSubdivisions = getScaleSubdivisions(a);
        ...
        if (showText()) {
            ...
            mTextUnitBelow = a.getString(R.styleable.GaugeView_textUnitBelow);
            mTextUnitLabel = a.getString(R.styleable.GaugeView_textUnitLabel);
    ...
    public void setDefaultNeedlePaths() {
        final float x = getDefaultNeedleX(), y = getDefaultNeedleY();
    ...
    public float getDefaultNeedleX() {
        return 0.5f;
    }
    ...
    protected void drawText(final Canvas canvas) {
...
//        final float textValueWidth = mTextValuePaint.measureText(textValue);
        ...
        final float startY = CENTER;

        drawTextOnCanvasWithMagnifier(canvas, textValue, startX, startY + .03f, mTextValuePaint);

        if (!TextUtils.isEmpty(mTextUnitBelow)) {
            drawTextOnCanvasWithMagnifier(canvas, mTextUnitBelow, startX, startY + 0.15f, mTextUnitPaint);
        }
        if (!TextUtils.isEmpty(mTextUnitLabel)) {
            drawTextOnCanvasWithMagnifier(canvas, mTextUnitLabel, startX, startY + 0.27f, mTextUnitPaint);
        }
    }

    public String getTextUnitLabel() {
        return mTextUnitLabel;
    }
...
//        final int startTick = Math.round((mScaleStartValue / mScaleEndValue) * mDivisions * mSubdivisions);
    ...
    protected String valueString(final float value) {
    ...
    protected static class ChangeEvent {
        ...
        public ChangeEvent(String eventName) {
            name = eventName;
        }
    }

    protected interface EventReceivedListener {
        void notifyOfEvent(ChangeEvent event);
    }

    private EventReceivedListener mEventReceivedListener;

    public void setEventListener(EventReceivedListener listener) {
        mEventReceivedListener = listener;
    }

    private float convertTimeToAcceleration(float timeMillis) {
        final float distanceToTravel = Math.abs(mTargetValue - mCurrentValue);
        return distanceToTravel / timeMillis * 1000;
    }

    public void setTimeToTarget(float timeMillis) {
        setAccelerationCoefficient(convertTimeToAcceleration(timeMillis));
    }

    private void setAccelerationCoefficient(float coefficient) {
        mAccelerationCoefficient = coefficient;
    }

    protected void computeCurrentValue() {
        //Log.i("info", String.format("velocity=%f, acceleration=%f, coefficient=%f", mNeedleVelocity,
        //mNeedleAcceleration, mAccelerationCoefficient));l
        ...
        if (-1 != mNeedleLastMoved) {
            ...
            if (direction > 0) {
                mCurrentValue += mAccelerationCoefficient * time;
            } else if (direction < 0) {
                mCurrentValue -= mAccelerationCoefficient * time;
            }
